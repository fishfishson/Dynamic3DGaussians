...
class MiniDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        camera_id = self.camera_names.index(self.camera_names[index])
        ...
        meta.ccm = torch.from_numpy(camera['ccm'].reshape(3, 4)).float()
...
def initialize_params(args, scene_radius):
    ...
    bg_pcd, bg_rgb, _, _ = load_pts(bg_ply_path)
    log(f'num of bg points: {bg_pcd.shape[0]}')
...
def train(args):
    ...
    if os.path.exists(join(args.data_root, 'optimized')):
        cameras = read_camera(join(args.data_root, 'optimized'))
        log('load optimized cameras')
    else:
        cameras = read_camera(args.data_root)
    with open(join(args.data_root, 'optimized', 'ccm.json')) as f:
        ccm = json.load(f)
    for k in cameras.keys():
        cameras[k]['ccm'] = np.asarray(ccm[k]).reshape(3, 4)
    ...
    log(blue(f'test_cameras: {test_cameras.keys()}'))
    ...
    log(blue(f'train_cameras: {train_cameras.keys()}'))
        ...
        for batch_id, batch in enumerate(dataloader):
            ...
            with torch.no_grad():
                ...
                if is_initial_timestep:
                    params, variables = densify(params, variables, optimizer, batch_id)
                optimizer.step()
...
if __name__ == "__main__":
    ...
    if args.exp_name == None:
        seq = args.data_root.split('/')[-1]
        log(f'use sequence name as exp_name: {seq}')
